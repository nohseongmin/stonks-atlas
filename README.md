...
```mermaid
flowchart TD
    A["620 시행<br/>국장 14 · 미장 467 · 코인 130"] --> B{"④ 벤치마크를<br/>넘는가"}
    B -->|"못 넘음<br/>대부분"| X1["기각"]
    B -->|"넘음<br/>소수"| C{"⑤ 비용 3 배에서도<br/>넘는가"}
    C -->|"못 넘음"| X2["기각"]
    C -->|"넘음"| D{"⑦ 잡음바닥<br/>1.98 을 넘는가"}
    D -->|"못 넘음<br/>전부"| X3["기각"]
    D -->|"넘음"| E["강건성 배터리 6 종"]
    E --> F["**완주 0 건**"]
...
xychart-beta
    ...
    bar [1.06, 1.06, 1.55, 1.55, 1.55, 1.55]
```

<sub>**선** = 참 실력 0 일 때 우연히 나올 최고 Sharpe · **막대** = 그 시점 실제 관측 최고</sub>
